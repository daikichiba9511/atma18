...
def main() -> None:
    ...
    df_feats = preprocess.add_feature(
        df_train, agg_cols=tuple([*cfg.use_cols, *[f"gbdt-pred-{c}" for c in target_cols]])
    )
    ...
    feature_cols = sorted([
        c for c in df_train.columns if c not in drop_cols + target_cols and not c.endswith("right")
    ])
    category_cols = sorted([c for c in feature_cols if df_train[c].dtype == pl.Categorical])

    logger.info("=================== COLS_FEATURES ======================")
    utils.pinfo(feature_cols)
    utils.pinfo(category_cols)
    logger.info("=========================================")
