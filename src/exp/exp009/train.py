...
def train_one_epoch(
    ...
    for batch_idx, batch in pbar:
        ...
        features = y[:, 18:].float()
        # 008との差分はAUXでの学習をしない, 入力にはAUXに使ってたカラムは使う
...
class MyTrainDataset(torch_data.Dataset[TrainBatch]):
    ...
    def __getitem__(self, idx: int) -> TrainBatch:
        ...
        if self.label_aux is not None:
            ...
            label_aux_cls = torch.tensor(label_aux[4:], dtype=torch.int32)
            label_aux = torch.tensor(label_aux[:4], dtype=torch.float32)
            label = torch.cat([label, label_aux, label_aux_cls], dim=0)
