...
class MyTrainDataset(torch_data.Dataset[TrainBatch]):
    ...
    def __getitem__(self, idx: int) -> TrainBatch:
        ...
        if self.label_aux is not None:
            label_aux = torch.tensor(self.label_aux[idx][:, :6])
            features = torch.tensor(self.label_aux[idx][:, 6:], dtype=torch.float32)
...
def init_dataloader(
    ...
    df_traffic_light = pl.read_csv(constants.DATA_DIR / "traffic_light.csv")
    ...
    original_cols = pl.read_csv(constants.DATA_DIR / "train.csv").columns
    df_feats = preprocess.cast_dtype(pl.concat([df_train, df_valid], how="vertical"))
    ...
    feature_cols = [c for c in df_feats.columns if c not in ["ID", *constants.TARGET_COLS, "base_path"]]
